...
const Films = () => {
  ...
  const movies = async () => {
    ...
    const list = await axios({
      ...
      params: param,
      url:
        "https://api.themoviedb.org/3/movie/popular?api_key=3cc05ada7e70628b8d1bf36e4d1f6fd7&language=ru-RU&page=" +
        page +
        "",
      config: {
        headers: {
          ...
          <div className="row">
            <div className="col-6 d-flex justify-content-end">
              <button onClick={previouspage}>
                <a href={"/films/" + page}>Предыдущая страница</a>
              </button>
            </div>
            <div className="col-6 d-flex justify-content-start">
              <button onClick={nextpage}>
                <a href={"/films/" + page}>Следующая страница</a>
              </button>
            </div>
          </div>
        </div>
